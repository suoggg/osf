...
class ConfusionMatrix:
    ...
    def __init__(self, nc, conf=0.25, iou_thres=0.45, task='detect', cube=False):
    ...
    def process_batch(self, detections, labels):
        ...
        if self.cube:
            detections = detections[detections[:, 14] > self.conf]
            gt_classes = labels[:, 0].int()
            detection_classes = detections[:, 15].int()
...
def ap_per_class(tp,
    ...
    # import pickle
    # with open('/data/users/qinhaolin01/OSFormer/visual-result/OSFormer_antiuav310.pkl', 'wb') as file:
    #     pickle.dump((px,py,ap,f1,p,r),file)
    if plot:

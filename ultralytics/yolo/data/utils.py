...
def verify_image_label(args):
    ...
    im_file, lb_file, cube, prefix, keypoint, num_cls, nkpt, ndim = args
    ...
    try:
        ...
        if os.path.isfile(lb_file):
            ...
            with open(lb_file) as f:
                ...
                if any((len(x) > 6 and len(x) < 13) for x in lb) and (not keypoint):  # is segment
            ...
            if nl:
                if keypoint:
                    ...
                    assert lb.shape[1] == 13 or lb.shape[1] == 5, f'labels require 13 or 5 columns, {lb.shape[1]} columns detected'
                    assert (lb[:, 1:] <= 1).all(), \
            ...
            nm = 1  # label missing
            if cube:
                lb = np.zeros((0, (5 + nkpt * ndim)), dtype=np.float32) if keypoint else np.zeros((0, 13), dtype=np.float32)
            else:

...
def letterbox(im, new_shape=(640, 640), color=(114, 114, 114), auto=True, scaleFill=False, scaleup=True, stride=32):
    ...
    if im.shape[2] <= 4:
        ...
        im = np.pad(im, ((top, bottom), (left, right), (0, 0)), mode='constant', constant_values=color[0] if isinstance(color, (list, tuple)) else color)
...
def random_perspective(im,
    ...
    if (border[0] != 0) or (border[1] != 0) or (M != np.eye(3)).any():  # image changed
        if im.shape[2] <= 4:
            ...
            im_parts = []
            for i in range(0, im.shape[2], 4):
                chunk = im[:, :, i:i+4]
                chunk_border_val = (114,) * chunk.shape[2]
                if perspective:
                    p = cv2.warpPerspective(chunk, M, dsize=(width, height), borderValue=chunk_border_val)
                else:
                    p = cv2.warpAffine(chunk, M[:2], dsize=(width, height), borderValue=chunk_border_val)
                im_parts.append(p)
            im = np.concatenate(im_parts, axis=2)

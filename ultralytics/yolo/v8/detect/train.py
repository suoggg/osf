...
class DetectionTrainer(BaseTrainer):
    ...
    def get_model(self, cfg=None, weights=None, verbose=True):
        ...
        model = DetectionModel(cfg, nc=self.data['nc'], verbose=verbose and RANK == -1, cube=self.args.cube, gray=self.args.gray)
        if weights:
            model.load(weights)
        return model
    ...
    def plot_training_labels(self):
        ...
        if self.args.cube:
            boxes = np.concatenate([lb['bboxes_t3'] for lb in self.train_loader.dataset.labels], 0)
        else:

...
class AutoBackend(nn.Module):
    ...
    def warmup(self, imgsz=(1, 3, 640, 640)):
        ...
        if any(warmup_types) and (self.device.type != 'cpu' or self.triton):
            # Use model's actual input channels if available
            ch = imgsz[1]
            if self.pt and hasattr(self.model, 'yaml') and 'ch' in self.model.yaml:
                ch = self.model.yaml['ch']
            elif hasattr(self, 'model') and hasattr(self.model, 'ch'):
                ch = self.model.ch
            
            if isinstance(ch, (list, tuple)):
                ch = ch[0]
            
            im = torch.empty((imgsz[0], ch, imgsz[2], imgsz[3]), dtype=torch.half if self.fp16 else torch.float, device=self.device)  # input
